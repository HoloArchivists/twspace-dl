...
class Login:
    ...
    def login(self) -> str:
        ...
        request_flow = self.session.post(
            self.task_url, headers=self._headers, json=self._js_instrumentation_data, timeout=30
        )
        try:
            self.flow_token = request_flow.json()["flow_token"]
        except KeyError as err:
        ...
        request_flow = self.session.post(
            self.task_url, headers=self._headers, json=self._user_identifier_sso_data, timeout=30
        )
        try:
            self.flow_token = request_flow.json()["flow_token"]
        except KeyError as err:
        ...
        request_flow = self.session.post(
            self.task_url, headers=self._headers, json=self._login_alternate_identifier, timeout=30
        ).json()
        ...
        # enter password
        request_flow = self.session.post(
            self.task_url, headers=self._headers, json=self._enter_password_data, timeout=30
        )
        ...
        request_flow = self.session.post(
            self.task_url, headers=self._headers, json=self._account_dup_check_data, timeout=30
        ).json()
        if "auth_token" in request_flow.cookies.keys():
            return str(request_flow.cookies["auth_token"])
        if "flow_token" in request_flow.keys():
